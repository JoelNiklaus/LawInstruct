...
class COLIEE(AbstractDataset):
	def __init__(self):
		super().__init__("COLIEE", "https://sites.ualberta.ca/~rabelo/COLIEE2022/")
	...
	def get_data(self):
		...
		for answer_language in answer_languages:
			with open(f"raw_data/coliee/task3/passage_entailment/train_{answer_language}.jsonl") as f:
		...
		for answer_language in answer_languages:
			with open(f"raw_data/coliee/task3/generate_entailed_question/train_{answer_language}.jsonl") as f:
				examples = [json.loads(x) for x in f.readlines()]
				for example in examples:
					text = example['text']
					yield self.build_data_point(prompt_language, answer_language, text, task_type, jurisdiction)
		...
		for answer_language in answer_languages:
			with open(f"raw_data/coliee/task4/train_{answer_language}.jsonl") as f:
				examples = [json.loads(x) for x in f.readlines()]
				for example in examples:
					text = example['text']
					yield self.build_data_point(prompt_langauge, answer_language, text, task_type, jurisdiction)
